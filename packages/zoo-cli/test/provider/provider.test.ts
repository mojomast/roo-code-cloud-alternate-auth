	...
	try {
		...
		await Instance.provide({
			...
			fn: async () => {
				...
				expect(
					Object.values(provider.models).every((model) => model.cost.input === 0 && model.cost.output === 0),
				).toBe(true)
			},
		})
	} finally {
...
test("provider api field sets model api.url", async () => {
	...
	await Instance.provide({
		...
		fn: async () => {
			...
			expect(providers[ProviderID.make("custom-api")].models["model-1"].api.url).toBe(
