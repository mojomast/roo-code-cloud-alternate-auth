...
test("Bedrock: model with us. prefix should not be double-prefixed", async () => {
	...
	await Instance.provide({
		...
		fn: async () => {
			...
			expect(
				providers[ProviderID.amazonBedrock].models["us.anthropic.claude-opus-4-5-20251101-v1:0"],
			).toBeDefined()
		},
	})
})
...
test("Bedrock: model with global. prefix should not be prefixed", async () => {
	...
	await Instance.provide({
		...
		fn: async () => {
			...
			expect(providers[ProviderID.amazonBedrock]).toBeDefined()
			expect(
				providers[ProviderID.amazonBedrock].models["global.anthropic.claude-opus-4-5-20251101-v1:0"],
			).toBeDefined()
		},
	})
})
...
test("Bedrock: model with eu. prefix should not be double-prefixed", async () => {
	...
	await Instance.provide({
		...
		fn: async () => {
			...
			expect(providers[ProviderID.amazonBedrock]).toBeDefined()
			expect(
				providers[ProviderID.amazonBedrock].models["eu.anthropic.claude-opus-4-5-20251101-v1:0"],
			).toBeDefined()
		},
	})
})
