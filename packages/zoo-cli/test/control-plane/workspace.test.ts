...
describe("workspace-old sync state", () => {
	...
	test("startWorkspaceSyncing starts only workspaces with sessions", async () => {
		await withInstance(async (dir) => {
			...
			expect(
				(await workspaceStatus()).find((item) => item.workspaceID === withoutSession.id)?.status,
			).toBeUndefined()
								...
								captured.events
									.filter(
										(event) =>
											...
											event.payload.type === WorkspaceOld.Event.Status.type,
									)
...
describe("workspace-old waitForSync", () => {
	...
	test("times out with the requested fence in the error message", async () => {
		await withInstance(async () => {
			...
			await expect(
				waitForWorkspaceSync(WorkspaceID.ascending("wrk_wait_timeout"), { [sessionID]: 1 }),
				...
				(dir) =>
					Effect.gen(function* () {
						...
						try {
							...
							expect(replay.every((call) => call.headers.get("authorization") === "Bearer restore")).toBe(
								true,
							)
							expect(
								replay.every((call) => call.headers.get("content-type") === "application/json"),
							).toBe(true)
							expect(replay.map((call) => (call.json as { events: unknown[] }).events.length)).toEqual([
								10, 10, 5,
							])
							expect(replay.map((call) => (call.json as { directory: string }).directory)).toEqual([
								dir,
								dir,
								dir,
							])
							...
							expect(
								(
									replay[2].json as { events: Array<{ seq: number; type: string; data: unknown }> }
								).events.at(-1),
							).toMatchObject({
								seq: 24,
								type: sessionUpdatedType(),
							...
							expect(
								captured.events
									.filter(
										(event) =>
											event.workspace === info.id &&
											event.payload.type === WorkspaceOld.Event.Restore.type,
									)
				...
				() =>
					Effect.gen(function* () {
						...
						registerAdapter(
							Instance.project.id,
							type,
							remoteAdapter(`${url}/null-dir`, { directory: null }).adapter,
						)
						const session = yield* sessionSvc.create({ title: "null dir" })
						replaceSessionEvents(session.id, 0)

						expect(
							yield* workspace.sessionRestore({ workspaceID: info.id, sessionID: session.id }),
						).toEqual({
							total: 1,
						})
					...
					Effect.gen(function* () {
						...
						try {
							...
							registerAdapter(
								Instance.project.id,
								type,
								remoteAdapter(`${url}/fail`, { directory: dir }).adapter,
							)
			...
			(dir) =>
				Effect.gen(function* () {
					...
					try {
						...
						expect((yield* sessionSvc.get(session.id)).workspaceID).toBe(info.id)
						expect(eventRows(session.id).map((row) => row.seq)).toEqual(
				...
				(dir) =>
					Effect.gen(function* () {
						...
						registerAdapter(
							Instance.project.id,
							type,
							remoteAdapter(`${url}/real`, { directory: dir }).adapter,
						)
						...
						expect(posted.map((event) => event.seq)).toEqual([
