...
function llm() {
	...
	return {
		push(
			stream: Stream.Stream<LLM.Event, unknown> | ((input: LLM.StreamInput) => Stream.Stream<LLM.Event, unknown>),
		) {
			queue.push(stream)
		},
		layer: Layer.succeed(
		...
		await Instance.provide({
			...
			fn: async () => {
				...
				const rt = runtime(
					...
					Plugin.defaultLayer,
					wide(),
					cfg({ tail_turns: 2, preserve_recent_tokens: 100 }),
				)
				try {
					const msgs = await svc.messages({ sessionID: session.id })
					const parent = msgs.at(-1)?.info.id
		...
		await Instance.provide({
			...
			fn: async () => {
				...
				const rt = runtime(
					...
					Plugin.defaultLayer,
					wide(),
					cfg({ tail_turns: 2, preserve_recent_tokens: 500 }),
				)
				try {
					const msgs = await svc.messages({ sessionID: session.id })
					const parent = msgs.at(-1)?.info.id
