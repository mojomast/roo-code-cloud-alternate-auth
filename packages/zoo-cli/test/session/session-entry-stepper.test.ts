...
describe("session-entry-stepper", () => {
	describe("stepWith", () => {
		...
		test("aggregates retry events onto the current assistant", () => {
			...
			expect(store.committed[0].retries).toEqual([
				retry(1, "rate limited", 1),
				retry(2, "provider overloaded", 2),
			])
		})
	})
	...
	describe("memory", () => {
		...
		test("appends committed and pending entries", () => {
			...
			const pending = SessionEntry.User.fromEvent(
				SessionEvent.Prompt.create({ text: "pending", timestamp: time(2) }),
			)
					...
					FastCheck.property(texts, texts, (a, b) => {
						const next = run(
							[
								SessionEvent.Text.Started.create({ timestamp: time(1) }),
								...a.map((x, i) =>
									SessionEvent.Text.Delta.create({ delta: x, timestamp: time(i + 2) }),
								),
								SessionEvent.Text.Started.create({ timestamp: time(a.length + 2) }),
								...b.map((x, i) =>
									SessionEvent.Text.Delta.create({ delta: x, timestamp: time(i + a.length + 3) }),
								),
							],
							active(),
						)
					...
					FastCheck.property(texts, text, (parts, end) => {
						const next = run(
							[
								SessionEvent.Reasoning.Started.create({ timestamp: time(1) }),
								...parts.map((x, i) =>
									SessionEvent.Reasoning.Delta.create({ delta: x, timestamp: time(i + 2) }),
								),
						...
						(callID, title, input, output, metadata, attachments, parts) => {
							const next = run(
								[
									...
									...parts.map((x, i) =>
										SessionEvent.Tool.Input.Delta.create({
											callID,
											delta: x,
											timestamp: time(i + 2),
										}),
				...
				FastCheck.assert(
					FastCheck.property(texts, (parts) => {
						const next = run([
							...
							...parts.map((x, i) =>
								SessionEvent.Text.Delta.create({ delta: x, timestamp: time(i + 3) }),
							),
							SessionEvent.Step.Ended.create({
								reason: "stop",
								cost: 1,
					...
					FastCheck.property(
						...
						(body, reason, end, input, title, output, metadata, attachments) => {
							...
							const next = run([
								...
								SessionEvent.Reasoning.Started.create({ timestamp: time(2) }),
								...reason.map((x, i) =>
									SessionEvent.Reasoning.Delta.create({ delta: x, timestamp: time(i + 3) }),
								),
