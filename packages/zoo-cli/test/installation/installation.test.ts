...
describe("installation", () => {
	describe("latest", () => {
		...
		test("reads npm versions via registry", async () => {
			...
			expect(calls).toContain(`https://registry.npmjs.org/@kilocode%2Fcli/${InstallationChannel}`) // kilocode_change
		})
		...
		test("reads brew tap info JSON via CLI", async () => {
			...
			const layer = testLayer(
				...
				(cmd, args) => {
					// kilocode_change start
					if (cmd === "brew" && args.includes("Kilo-Org/tap/kilo") && args.includes("--formula"))
						return "kilo"
					// kilocode_change end
