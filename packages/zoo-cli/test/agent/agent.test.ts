...
test("explore agent asks for external directories and allows whitelisted external paths", async () => {
	...
	await Instance.provide({
		...
		fn: async () => {
			const explore = await load(tmp.path, (svc) => svc.get("explore"))
			expect(explore).toBeDefined()
			expect(Permission.evaluate("external_directory", "/some/other/path", explore!.permission).action).toBe(
				"ask",
			)
			expect(Permission.evaluate("external_directory", Truncate.GLOB, explore!.permission).action).toBe("allow")
			expect(
				Permission.evaluate("external_directory", path.join(Global.Path.tmp, "agent-work"), explore!.permission)
					.action,
			).toBe("allow")
		},
	})
...
test("global tmp directory children are allowed for external_directory", async () => {
	...
	await Instance.provide({
		...
		fn: async () => {
			...
			expect(
				Permission.evaluate("external_directory", path.join(Global.Path.tmp, "scratch"), build!.permission)
					.action,
			).toBe("allow")
