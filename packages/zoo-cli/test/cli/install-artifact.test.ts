...
const wrapper = path.join(root, "bin", "zoo")
...
describe("npm install artifact behavior", () => {
	test("keeps the CLI wrapper contract", async () => {
		...
		expect(text).toContain("const envPath = process.env.ZOO_BIN_PATH")
		expect(text).toContain('const base = "@zoo-code/cli-" + platform + "-" + arch')
	...
	test("links npm bin commands to the wrapper during local install", async () => {
		...
		const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "zoo-install-artifact-"))
		try {
			...
			await fs.copyFile(wrapper, path.join(bin, "zoo"))
			await Bun.write(
				path.join(pkg, "package.json"),
				JSON.stringify(
					{
						name: "zoo-install-artifact-repro",
						version: "1.0.0",
						bin: {
							zoo: "./bin/zoo",
							"zoo-code": "./bin/zoo",
			...
			const hidden = path.join(prefix, "node_modules", ".bin", ".zoo")
