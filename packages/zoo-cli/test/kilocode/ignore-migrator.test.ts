...
# Comment 2`
  ...
  *.env  `
	...
	describe("migrate", () => {
		test("returns empty permission for project without ignore files", async () => {
		...
		test("loads project .zooignore", async () => {
			await using tmp = await tmpdir({
				init: async (dir) => {
		...
		test("keeps .kilocodeignore as lower-priority migration fallback", async () => {
			await using tmp = await tmpdir({
				init: async (dir) => {
					await Bun.write(path.join(dir, ".kilocodeignore"), "legacy/")
					await Bun.write(path.join(dir, ".zooignore"), "!legacy/public.txt")
				},
			})

			const result = await IgnoreMigrator.migrate({ projectDir: tmp.path, skipGlobalPaths: true })
			const readRules = result.permission.read as Record<string, string>

			expect(readRules["legacy/*"]).toBe("deny")
			expect(readRules["legacy/public.txt"]).toBe("allow")
		})
