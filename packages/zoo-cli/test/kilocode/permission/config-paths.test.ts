...
describe("ConfigProtection.isRequest", () => {
	...
	const zooConfig = path.resolve(ConfigPaths.zooGlobalConfigDir())
	...
	test("returns true for bash external_directory targeting Zoo global config", () => {
		const result = ConfigProtection.isRequest({
			permission: "external_directory",
			patterns: [zooConfig + "/*"],
			metadata: {},
	...
	test("returns true for edit targeting root config files", () => {
		for (const file of ["zoo.json", "zoo.jsonc", ".zooignore", "kilo.json", "kilo.jsonc", "AGENTS.md"]) {
			const result = ConfigProtection.isRequest({
				permission: "edit",
				patterns: [file],
			})
