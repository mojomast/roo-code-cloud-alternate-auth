		...
		await Instance.provide({
			...
			fn: async () => {
				...
				await Permission.reply({
					requestID: PermissionID.make("permission_bash_external_write"),
					reply: "reject",
				})
	...
	test("emits read access metadata for cat external files", async () => {
		...
		await Instance.provide({
			...
			fn: async () => {
				...
				await expect(
					Effect.runPromise(
						bash.execute({ command, description: "Read external file" }, capture(requests, err)),
					),
		...
		await Instance.provide({
			...
			fn: async () => {
				...
				for (const command of commands) {
					...
					await expect(
						Effect.runPromise(
							bash.execute({ command, description: "Mutate external file" }, capture(requests, err)),
						),
					).rejects.toThrow(err.message)
			...
			fn: async () => {
				...
				for (const command of commands) {
					...
					await expect(
						Effect.runPromise(
							bash.execute(
								{ command, description: "Read then write external file" },
