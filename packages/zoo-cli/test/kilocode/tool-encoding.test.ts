...
const encodeBytes = (text: string, encoding: string): Buffer => {
	...
	if (lower === "utf-32le")
		return Buffer.concat([Buffer.from([0xff, 0xfe, 0x00, 0x00]), iconv.encode(text, encoding)])
	if (lower === "utf-32be")
		return Buffer.concat([Buffer.from([0x00, 0x00, 0xfe, 0xff]), iconv.encode(text, encoding)])
