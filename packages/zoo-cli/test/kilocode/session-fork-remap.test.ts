		...
		async () => {
			...
			await Instance.provide({
				...
				fn: async () => {
					...
					const forkedChildID = (tools[0].state as unknown as { metadata: { sessionId: string } }).metadata
						.sessionId
					...
					const forkedGrandchildID = (childTools[0].state as unknown as { metadata: { sessionId: string } })
						.metadata.sessionId
