...
describe("util.effect-zod", () => {
	...
	test("Permission schemas match original Zod equivalents", () => {
		...
		expect(bridged.required?.sort()).toEqual(
			["id", "sessionID", "permission", "patterns", "metadata", "always"].sort(),
		)
	...
	describe("optimizations", () => {
		...
		test("multiple checks run in a single refinement layer (all fire on one value)", () => {
			...
			expect(neg.error!.issues.map((i) => i.message)).toEqual(
				expect.arrayContaining(["not positive", "not even"]),
			)
