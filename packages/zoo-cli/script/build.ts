...
async function copyTreeSitterWasms(outputDir: string) {
	...
	await Promise.all(
		languageWasmFiles.map((file) =>
			fs.promises.copyFile(path.join(languageWasmDir, file), path.join(targetDir, file)),
		),
	)
...
for (const item of targets) {
	...
	await Bun.build({
		...
		compile: {
			...
			outfile: `dist/${name}/bin/zoo`, // kilocode_change
			execArgv: [`--user-agent=zoo/${Script.version}`, "--use-system-ca", "--"], // kilocode_change
			windows: {},
		},
	...
	if (item.os === "linux") {
		...
		if (interpreter) {
			try {
				await $`patchelf --set-interpreter ${interpreter} dist/${name}/bin/zoo`
	...
	if (item.os === process.platform && item.arch === process.arch && !item.abi) {
		const binaryPath = `dist/${name}/bin/zoo` // kilocode_change
...
if (Script.release) {
	...
	for (const key of Object.keys(binaries)) {
		const archive = key.replace(pkg.name, "zoo") // kilocode_change
