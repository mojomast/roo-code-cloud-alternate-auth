			...
			yield* sessions
				...
				.pipe(
					Effect.catchCause((cause) =>
						elog.error("failed to generate title", { error: Cause.squash(cause) }),
					),
				)
		})
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
				...
				.pipe(
					...
					Effect.onInterrupt(() =>
						Effect.gen(function* () {
							...
							const cid = childID()
							if (cid) {
								assistantMessage.cost = yield* KiloCostPropagation.childCost(
									sessions,
									SessionID.make(cid),
								)
							}
		...
		const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (input: PromptInput) {
			...
			const resolvePart: (part: PromptInput["parts"][number]) => Effect.Effect<Draft<MessageV2.Part>[]> =
				Effect.fn("SessionPrompt.resolveUserPart")(function* (part) {
					if (part.type === "file") {
						...
						switch (url.protocol) {
							...
							case "file:": {
								...
								if (mime === "application/x-directory") {
									...
									const exit = yield* execRead(args, { includeDirectoryFiles: true }).pipe(
										Effect.exit,
									) // kilocode_change inline folder files
		...
		const loop: (input: LoopInput) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.loop")(
