...
function custom(dep: CustomDep): Record<string, CustomLoader> {
	return {
		...
		"cloudflare-workers-ai": Effect.fnUntraced(function* (input: Info) {
			...
			const env = yield* dep.env()
			const accountId =
				env["CLOUDFLARE_ACCOUNT_ID"] || (auth?.type === "api" ? auth.metadata?.accountId : undefined)
			if (!accountId)
				return {
					autoload: false,
					async getModel() {
		...
		"cloudflare-ai-gateway": Effect.fnUntraced(function* (input: Info) {
			...
			const accountId =
				env["CLOUDFLARE_ACCOUNT_ID"] || (auth?.type === "api" ? auth.metadata?.accountId : undefined)
			const gateway =
				env["CLOUDFLARE_GATEWAY_ID"] || (auth?.type === "api" ? auth.metadata?.gatewayId : undefined)
			...
			if (!apiToken) {
				throw new Error(
					"CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
						"Set it via environment variable or run `zoo auth cloudflare-ai-gateway`.", // kilocode_change
				)
			}
			...
			Effect.gen(function* () {
				...
				for (const [id, provider] of Object.entries(providers)) {
					...
					for (const [modelID, model] of Object.entries(provider.models)) {
						...
						if (model.status === "alpha" && !Flag.KILO_ENABLE_EXPERIMENTAL_MODELS)
							delete provider.models[modelID]
