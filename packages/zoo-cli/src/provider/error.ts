...
function message(providerID: ProviderID, e: APICallError) {
	return iife(() => {
		...
		if (/^\s*<!doctype|^\s*<html/i.test(e.responseBody)) {
			if (e.statusCode === 401) {
				return "Unauthorized: request was blocked by a gateway or proxy. Your authentication token may be missing or expired — try running `zoo auth login <your provider URL>` to re-authenticate." // kilocode_change
			}
			if (e.statusCode === 403) {
