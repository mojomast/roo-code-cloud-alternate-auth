...
export async function create(input: { serverID: string; server: LSPServer.Handle; root: string; directory: string }) {
	...
	function documentPullState() {
		...
		return {
			documentIdentifiers: [
				...new Set(
					documentRegistrations.flatMap((registration) => registration.registerOptions?.identifier ?? []),
				),
			],
	...
	function workspacePullState() {
		...
		return {
			workspaceIdentifiers: [
				...new Set(
					workspaceRegistrations.flatMap((registration) => registration.registerOptions?.identifier ?? []),
				),
			],
