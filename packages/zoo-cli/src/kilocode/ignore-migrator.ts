...
export namespace IgnoreMigrator {
	...
	const ZOOIGNORE_FILE = ".zooignore"
	...
	/**
	 * Load patterns from a Zoo/Kilo ignore file
	 */
	...
	/**
	 * Migrate .zooignore to Opencode permission config, with .kilocodeignore as a migration fallback.
	 */
	export async function migrate(options: {
		...
		// 2. Load legacy project .kilocodeignore first (lower priority), then preferred .zooignore.
		const legacyProjectPatterns = await loadIgnoreFile(
			path.join(options.projectDir, KILOCODEIGNORE_FILE),
			"project",
		)
		allPatterns.push(...legacyProjectPatterns)
		const projectIgnorePath = path.join(options.projectDir, ZOOIGNORE_FILE)
		const projectPatterns = await loadIgnoreFile(projectIgnorePath, "project")
		allPatterns.push(...projectPatterns)

		if (projectPatterns.length > 0) {
			log.debug("loaded project .zooignore", { count: projectPatterns.length })
		}
	...
	/**
	 * Load .zooignore/.kilocodeignore and return permission config.
	...
	export async function loadIgnoreConfig(
		...
		try {
			...
			if (result.patternCount > 0) {
				log.info("loaded .zooignore patterns", {
			...
			log.warn("failed to load .zooignore", { error: err })
			return {}
		}
	}
}
