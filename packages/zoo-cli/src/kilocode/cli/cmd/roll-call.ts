	...
	await Instance.provide({
		...
		async fn() {
			...
			if (models.length === 0) {
				if (!args.quiet && !structured)
					UI.println(
						`${color(UI.Style.TEXT_WARNING)}No models to test after filtering.${color(UI.Style.TEXT_NORMAL)}`,
					)
			...
			if (!args.quiet && !structured) {
				UI.println(
					`${color(UI.Style.TEXT_INFO)}Prompting ${models.length} models...${color(UI.Style.TEXT_NORMAL)}`,
				)
			}
			...
			const run = async (item: (typeof models)[0]) => {
				...
				if (result.access) {
					UI.println(
						`${color(UI.Style.TEXT_SUCCESS)}✔${color(UI.Style.TEXT_NORMAL)} ${name} - ${result.latency}ms`,
					)
					return
				}
				UI.println(
