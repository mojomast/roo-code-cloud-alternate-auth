...
export namespace KiloCompactionChunks {
	...
	function large(input: { messages: MessageV2.WithParts[]; model: Provider.Model; size: number }) {
		return estimate({ messages: input.messages, model: input.model }).pipe(
			Effect.map((count) => count > input.size),
		)
	}
	...
	function run(input: Input & { data: LLM.StreamInput["messages"]; text: string }) {
		return Effect.gen(function* () {
			const msg = yield* input.session.updateMessage(
				assistant({ base: input.target, sessionID: input.sessionID }),
			)
			...
			const out = yield* Effect.gen(function* () {
				...
				Effect.ensuring(
					input.session
						.removeMessage({ sessionID: input.sessionID, messageID: worker.message.id })
						.pipe(Effect.ignore),
				),
			)
