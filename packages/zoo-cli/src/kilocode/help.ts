...
async function getSubcommands(
	...
	const inst = yargs([]).scriptName(`zoo ${name}`).wrap(null)
	builder(inst)
	...
	try {
		...
		for (const [sub, handler] of Object.entries(handlers as Record<string, any>)) {
			...
			const full = `${name} ${sub}`
			const subInst = yargs([]).scriptName(`zoo ${full}`).wrap(null)
...
function formatMarkdown(
	...
	for (const section of sections) {
		parts.push(`## ${section.name ? `zoo ${section.name}` : "zoo"}`)
		parts.push("")
		if (section.hidden) {
...
function formatText(
	...
	for (const section of sections) {
		parts.push(rule)
		const display = section.name ? `zoo ${section.name}` : "zoo"
		...
		for (const sub of section.subs) {
			const sublabel = sub.hidden ? `--- zoo ${sub.name} [internal] ---` : `--- zoo ${sub.name} ---`
			parts.push(sublabel)
			parts.push("")
			parts.push(sub.help)
			parts.push("")
...
export async function generateCommandTable(options?: { commands?: Cmd[] }) {
	...
	for (const cmd of cmds) {
		...
		const display = raw.startsWith("$0") ? "zoo" + raw.slice(2) : "zoo " + raw
