...
export namespace ConfigProtection {
	 ...
	 * Matches .zoo/ and legacy .kilo/.kilocode/.opencode at any depth within the project.
	 */
	const CONFIG_DIRS = [".zoo/", ".kilo/", ".kilocode/", ".opencode/"]
	...
	function configs(): string[] {
		return Array.from(
			new Set([
				ConfigPaths.zooGlobalConfigDir(),
				Global.Path.config,
	...
	function fallback(p: string): boolean {
		...
		return keys(p).some(
			(key) =>
				key.endsWith("/config/zoo-code") ||
				key.includes("/config/zoo-code/") ||
	...
	export function isAbsolute(filepath: string): boolean {
		...
		// ~/.config/zoo-code/ plus legacy global config dirs
