...
export const USER_AGENT = `zoo/${InstallationChannel}/${InstallationVersion}/${Flag.KILO_CLIENT}` // kilocode_change
			...
			const result: Interface = {
				...
				method: Effect.fn("Installation.method")(function* () {
					...
					const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
						...
						// kilocode_change start
						{ name: "brew", command: () => text(["brew", "list", "--formula", "zoo"]) },
						{ name: "scoop", command: () => text(["scoop", "list", "zoo"]) },
						{ name: "choco", command: () => text(["choco", "list", "--limit-output", "zoo"]) },
						// kilocode_change end
					]
				...
				upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
					let upgradeResult:
						| { code: ChildProcessSpawner.ExitCode; stdout: string; stderr: string }
						| undefined
					switch (m) {
						case "curl":
							...
							break
						case "npm":
							upgradeResult = yield* run(["npm", "install", "-g", `@zoo-code/cli@${target}`]) // kilocode_change
							break
						case "pnpm":
							upgradeResult = yield* run(["pnpm", "install", "-g", `@zoo-code/cli@${target}`]) // kilocode_change
							break
						case "bun":
							upgradeResult = yield* run(["bun", "install", "-g", `@zoo-code/cli@${target}`]) // kilocode_change
							break
						case "brew": {
							const formula = yield* getBrewFormula()
						...
						case "choco":
							...
							break
						case "scoop":
							upgradeResult = yield* run(["scoop", "install", `zoo@${target}`]) // kilocode_change
							break
						default:
					...
					if (!upgradeResult || upgradeResult.code !== 0) {
						const stderr =
							m === "choco" ? "not running from an elevated command shell" : upgradeResult?.stderr || ""
