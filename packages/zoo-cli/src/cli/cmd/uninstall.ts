...
export const UninstallCommand = {
	command: "uninstall",
	describe: "uninstall zoo and remove all related files", // kilocode_change
	builder: (yargs: Argv) =>
		yargs
			.option("keep-config", {
				alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
	...
	if (method !== "curl" && method !== "unknown") {
		const cmds: Record<string, string> = {
			npm: "npm uninstall -g @zoo-code/cli", // kilocode_change
			pnpm: "pnpm uninstall -g @zoo-code/cli", // kilocode_change
			bun: "bun remove -g @zoo-code/cli", // kilocode_change
			yarn: "yarn global remove @zoo-code/cli", // kilocode_change
	...
	for (const line of lines) {
		...
		if (
			(trimmed.startsWith("export PATH=") &&
				(trimmed.includes(".opencode/bin") || trimmed.includes(".kilo/bin"))) ||
