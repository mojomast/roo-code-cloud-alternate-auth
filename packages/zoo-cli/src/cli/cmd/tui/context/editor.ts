	...
	init: (props: { WebSocketImpl?: typeof WebSocket }) => {
		...
		const connect = () => {
			...
			current.addEventListener("message", (event) => {
				...
				const mention =
					message.method === "at_mentioned" ? EditorMentionSchema.safeParse(message.params) : undefined
				if (mention?.success) {
				...
				const initialize =
					method === "initialize" ? EditorServerInfoSchema.safeParse(message.result) : undefined
