			...
			async fn() {
				{
					...
					async function getAppInfo() {
						...
						const info = await AppRuntime.runPromise(
							Git.Service.use((git) =>
								git.run(["remote", "get-url", "origin"], { cwd: Instance.worktree }),
							),
						).then((x) => x.text().trim())
					...
					async function installGitHubApp() {
						...
						async function getInstallation() {
							...
							return await fetch(
								`https://api.kilo.ai/api/integrations/github/check-installation?owner=${app.owner}`,
							)
								.then((res) => res.json())
		...
		await bootstrap(process.cwd(), async () => {
			...
			const gitText = async (args: string[]) => {
				const result = await AppRuntime.runPromise(
					Git.Service.use((git) => git.run(args, { cwd: Instance.worktree })),
				)
