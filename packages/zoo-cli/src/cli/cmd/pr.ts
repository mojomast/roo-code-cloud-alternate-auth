...
export const PrCommand = cmd({
	...
	describe: "fetch and checkout a GitHub PR branch, then run zoo", // kilocode_change
	builder: (yargs) =>
	...
	async handler(args) {
		await Instance.provide({
			...
			async fn() {
					...
					if (prInfoText.trim()) {
						...
						if (prInfo && prInfo.isCrossRepository && prInfo.headRepository && prInfo.headRepositoryOwner) {
							...
							await AppRuntime.runPromise(
								Git.Service.use((git) =>
									git.run(
										["branch", `--set-upstream-to=${remoteName}/${headRefName}`, localBranchName],
										{
											cwd: Instance.worktree,
										},
									),
				...
				if (code !== 0) throw new Error(`zoo exited with code ${code}`) // kilocode_change
			},
		})
	},
})
