...
function normalizeLegacyOperation(operation: OpenApiOperation, path: string, method: string) {
	...
	if ((path !== "/session/{sessionID}/message" && path !== "/session/{sessionID}/command") || method !== "post")
		return
...
function pathParameterSchema(route: string, name: string) {
	...
	if (name === "id" && route.startsWith("DELETE /experimental/workspace/"))
		return { type: "string", pattern: "^wrk.*" }
